...
{
	if (execve(tmp_cmd->sim_cmd[0], tmp_cmd->sim_cmd, shell->env) < 1)
	{
		perror (0);
		exit(127);
	}
...
void	ft_close_main(t_shell *shell, pid_t *cmd, int **pipe_fds)
{
	int	i;
	int	status;
...
int ft_fork(t_shell *shell, t_cmd *tmp_cmd, int i, int *pipe_fds[2])
{
	...
	if (cmd[i] == 0)
	{			
		ft_close_pipes(shell, pipe_fds, i);
		if (i < shell->cmd_nbr - 1)
			ft_deal_middle_command(tmp_cmd, pipe_fds[i][0], pipe_fds[i + 1][1]);	
		else	
			ft_deal_middle_command(tmp_cmd, pipe_fds[i][0], pipe_fds[i + 1][1]);
		ft_exec(shell, tmp_cmd, pipe_fds[i][0], pipe_fds[i + 1][1]);
		exit(EXIT_SUCCESS);
	}
	ft_close_main(shell, cmd, pipe_fds);
	return (0);
}


void ft_run_multiple_commands(t_shell * shell)
{
	t_cmd	*tmp_cmd;
	...
	while (i < shell->cmd_nbr)
	{
		pipe(pipe_fds[i]ft_create_pipes(shell, pipe_fds[i]);
		ft_fork(shell, tmp_cmd, i, pipe_fds);
		i++;
